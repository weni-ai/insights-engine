...
class TestConversationsMetricsViewSetAsAuthenticatedUser(
    ...
    def test_cannot_get_added_to_cart_metrics_without_project_uuid(self):
        response = self.get_added_to_cart_metrics({})

        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data["project_uuid"][0].code, "required")

    def test_cannot_get_added_to_cart_metrics_without_permission(self):
        response = self.get_added_to_cart_metrics({"project_uuid": self.project.uuid})

        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    @with_project_auth
    def test_cannot_get_added_to_cart_metrics_without_required_params(self):
        response = self.get_added_to_cart_metrics({"project_uuid": self.project.uuid})
    ...
    @with_project_auth
    @patch(
        "insights.metrics.conversations.services.ConversationsMetricsService.get_added_to_cart_metrics"
    )
    def test_get_added_to_cart_metrics(self, mock_get_added_to_cart_metrics):
        mock_get_added_to_cart_metrics.return_value = {
            "results": [
                {"label": "azeite", "value": 85.0, "full_value": 17},
                {"label": "arroz", "value": 15.0, "full_value": 3},
            ]
    ...
    def test_get_added_to_cart_metrics_returns_500_on_service_error(
        self, mock_get_added_to_cart_metrics
    ):
        mock_get_added_to_cart_metrics.side_effect = RuntimeError("Service error")
