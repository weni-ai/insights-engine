...
class TestInternalConversationsMetricsViewSetAsAuthenticatedUser(
    BaseTestInternalConversationsMetricsViewSet
):
    def setUp(self):
    ...
    def test_cannot_get_project_ai_csat_metrics_when_widget_not_found(self):
        response = self.get_project_ai_csat_metrics(
            {
                "project_uuid": self.project.uuid,
                "start_date": "2024-01-01",
        ...
        self.assertEqual(
            response.data["error"], "AI CSAT metrics not found for this project"
        )
