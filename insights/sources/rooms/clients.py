...
relation_schema = {
    "users": {"field_name": "user_id", "table_alias": "r"},
    "project": {"field_name": "uuid", "table_alias": "p"},
    "tag": {"field_name": "uuid", "table_alias": "tg"},
...
def get_joins_from_schema(field):
    ...
    if "sector" == field or "project" == field:
        joins.add("INNER JOIN public.queues_queue AS q ON q.uuid=r.queue_id")
    if "project" == field:
        joins.add("INNER JOIN public.sectors_sector AS s ON s.uuid=q.sector_id")
        joins.add("INNER JOIN public.projects_project AS p ON p.uuid=s.project_id")
    if "tag" == field:
        joins.add("INNER JOIN public.rooms_room_tags AS tg ON tg.room_id=r.uuid")
