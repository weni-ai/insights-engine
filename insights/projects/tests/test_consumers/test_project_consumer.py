...
class TestOldProjectConsumer(TestCase):
    ...
    @patch(
        "insights.projects.consumers.project_consumer.ProjectAuthCreationUseCase"
    )
...
class TestWeniEDAProjectConsumer(TestCase):
    ...
    @patch("insights.projects.consumers.project_consumer.ProjectsUseCase")
    def test_consume_processes_message_and_acks(
        self, mock_projects_uc, mock_auth_uc
    ):
        project_uuid = str(uuid4())
        org_uuid = str(uuid4())
        mock_project = MagicMock()
...
class TestUpdateProjectConsumer(TestCase):
    ...
    @patch(
        "insights.projects.consumers.update_project_consumer.UpdateProjectUseCase"
    )
    ...
    @patch(
        "insights.projects.consumers.update_project_consumer.capture_exception"
    )
    @patch(
        "insights.projects.consumers.update_project_consumer.UpdateProjectUseCase"
    )
    def test_consume_rejects_on_exception(self, mock_update_uc, mock_capture):
        mock_update_uc.return_value.execute.side_effect = RuntimeError(
            "not found"
        )
