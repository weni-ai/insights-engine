...
class HumanSupportDashboardService:
    ...
    def _resolve_revenue_period(self, normalized: dict) -> tuple:
        """
        Returns the period to be reported, falling back to the current day.
        """
        if normalized.get("start_date") and normalized.get("end_date"):
        ...
        end_datetime = dj_timezone.now().astimezone(project_tz)

        return start_datetime, end_datetime

    def _resolve_comparison_period(
        self, normalized: dict, start_date: datetime, end_date: datetime
    ) -> tuple:
        """
        Returns the period to compare against, falling back to the preceding
        period of the same length.
        """
        if normalized.get("comparison_start_date") and normalized.get(
            "comparison_end_date"
        ):
            return (
                normalized["comparison_start_date"],
                normalized["comparison_end_date"],
            )
    ...
    def _build_revenue_filters(self, normalized: dict) -> dict:
        params: dict = {"project": str(self.project.uuid)}

        revenue_filters_mapping = {
            "sectors": "sector",
            "queues": "queue",
            "tags": "tag",
            "channels": "channels",
        ...
        for filter_key, param in revenue_filters_mapping.items():
            if not (value := normalized.get(filter_key)):
                continue
    ...
    def get_total_revenue(self, filters: dict | None = None) -> dict:
        """
        Returns the revenue generated by the human support rooms in the period,
        along with the revenue of the comparison period and the variation
        between them.
        ...
        current = self.revenue_source.get_revenue(
            {
                **base,
        ...
        previous = self.revenue_source.get_revenue(
            {
                **base,
